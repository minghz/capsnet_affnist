    ...
    def __call__(self, input, kernel_size=None, stride=None):
        ...
        if self.layer_type == 'CONV':
            ...
            if not self.with_routing:
                ...
                assert capsules.get_shape() == [cfg.batch_size, 1440, 8, 1]
                return(capsules)
...
def routing(input, b_IJ):
    ...
    W = tf.get_variable('Weight', shape=(1, 1440, 160, 8, 1), dtype=tf.float32,
    ...
    u_hat = tf.reshape(u_hat, shape=[-1, 1440, 10, 16, 1])
    assert u_hat.get_shape() == [cfg.batch_size, 1440, 10, 16, 1]
    ...
    for r_iter in range(cfg.iter_routing):
        with tf.variable_scope('iter_' + str(r_iter)):
            ...
            if r_iter == cfg.iter_routing - 1:
                ...
                v_J_tiled = tf.tile(v_J, [1, 1440, 1, 1, 1])
                u_produce_v = reduce_sum(u_hat_stopped * v_J_tiled, axis=3, keepdims=True)
                assert u_produce_v.get_shape() == [cfg.batch_size, 1440, 10, 1, 1]
